...
window.startProfitLands=()=>{resetForMode(state.mode,state.playType);$('homeScreen').classList.remove('active');$('gameScreen').classList.add('active');render();startTimer();window.profitLandsPlatform?.gameplayStart()};
window.resumeProfitLands=()=>{if(!state.dayEndsAt)state.dayEndsAt=Date.now()+(MODES[state.mode]?.daySeconds||900)*1000;state.started=true;$('homeScreen').classList.remove('active');$('gameScreen').classList.add('active');render();startTimer();window.profitLandsPlatform?.gameplayStart()};
