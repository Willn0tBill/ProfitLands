...
function money(n){if(Math.abs(n)>=1e12)return '
function save(){
...
function renderStocks(){$('stockList').innerHTML=STOCKS.map(s=>{const h=state.stocks[s.id];const pct=s._change||0;return `<div class="stock" data-stock="${s.id}"><div><b>${s.name}</b><small>${s.ticker} · ${s.sector}</small></div><div class="stock-price">${stockMoney(s.price)}</div><div class="change ${pct>=0?'up':'down'}">${pct>=0?'+':''}${pct.toFixed(1)}%</div></div>`}).join('');document.querySelectorAll('.stock').forEach(el=>el.onclick=()=>trade(el.dataset.stock))}
...
function trade(id){
 ...
 $('tradeContent').innerHTML=`<span class="eyebrow">STOCK MARKET</span><h2>${s.name}</h2><p class="modal-sub">${s.ticker} · ${s.sector} · Current price ${stockMoney(s.price)} · Risk: ${s.risk}</p><div class="trade-balance"><span>Cash</span><b>${money(state.cash)}</b></div><div class="trade-actions"><button class="trade-action" data-trade="buy"><b>Buy 1 Share</b><small>${stockMoney(s.price)}</small></button><button class="trade-action" data-trade="sell"><b>Sell 1 Share</b><small>Own: ${state.stocks[id]?.shares||0}</small></button></div>`;
...
+(n/1e12).toFixed(2)+'T';if(Math.abs(n)>=1e9)return '
function save(){
  try{if(state.started)syncTime()}catch(e){}
  captureMarket();
  state.savedAt=Date.now();
...
window.addEventListener('pagehide',saveLocalSnapshot);
document.addEventListener('visibilitychange',()=>{if(document.visibilityState==='hidden')saveLocalSnapshot()});
+(n/1e9).toFixed(2)+'B';if(Math.abs(n)>=1e6)return '
function save(){
  try{if(state.started)syncTime()}catch(e){}
  captureMarket();
  state.savedAt=Date.now();
...
window.addEventListener('pagehide',saveLocalSnapshot);
document.addEventListener('visibilitychange',()=>{if(document.visibilityState==='hidden')saveLocalSnapshot()});
+(n/1e6).toFixed(2)+'M';if(Math.abs(n)>=1e3)return '
function save(){
  try{if(state.started)syncTime()}catch(e){}
  captureMarket();
  state.savedAt=Date.now();
...
window.addEventListener('pagehide',saveLocalSnapshot);
document.addEventListener('visibilitychange',()=>{if(document.visibilityState==='hidden')saveLocalSnapshot()});
+Math.round(n).toLocaleString();return '
function save(){
  try{if(state.started)syncTime()}catch(e){}
  captureMarket();
  state.savedAt=Date.now();
